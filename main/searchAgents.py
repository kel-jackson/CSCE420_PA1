...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        cornerStatus = [False, False, False, False]

        # Initialize visit status of corners
        for i in range(0, len(self.corners)):
            if self.corners[i] == self.startingPosition:
                cornerStatus[i] = True
        
        self.startingState = (self.startingPosition, tuple(cornerStatus))
    ...
    def isGoalState(self, state):
        ...
        # Goal is all corners reached, so if corner missed = not goal
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not hitsWall:
                ...
                # Create final state and add to successors
                finalState = (nextState, tuple(cornerStatus))
                successors.append((finalState, action, 1))
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # Get current position and corners not visited
    ...
    for i in range(0, len(stateCorners)):
        if stateCorners[i] == False:
            remainingCorners += [corners[i]]

    # If corners left, calculate heuristic
    if len(remainingCorners) > 0:

        # Find farthest point to simplify problem
        farthestCost = util.manhattanDistance(currPosition, remainingCorners[0])

        # Check which corner is farthest
        for j in range(1, len(remainingCorners)):
            newCost = util.manhattanDistance(currPosition, remainingCorners[j])

            if newCost > farthestCost:
                farthestCost = newCost

        # heuristic = cost to farthest point
        return farthestCost
...
def foodHeuristic(state, problem):
    ...
    # Get current position and food not visited
    currPosition = state[0]
    foodPositions = foodGrid.asList()

    # If food left, calculate heuristic
    if len(foodPositions) > 0:

        # Find farthest point to simplify problem
        farthestCost = mazeDistance(currPosition, foodPositions[0], problem.startingGameState)

        # Check which food is farthest
        for j in range(1, len(foodPositions)):
            newCost = mazeDistance(currPosition, foodPositions[j], problem.startingGameState)
